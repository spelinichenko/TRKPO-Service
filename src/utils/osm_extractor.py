...
def get_spb_food_places_by_district(spb_district) -> gpd.GeoDataFrame:
    tags = {"amenity": ["cafe", "fast_food", "food_court", "restaurant"]}
    data = ox.features_from_polygon(spb_district.geometry.iloc[0], tags=tags).reset_index()
    return data


def spatial_join_food_places_and_district(
    food_places: gpd.GeoDataFrame, district_gdf: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
    return (
        gpd.sjoin(food_places, district_gdf)
        .rename(columns={"name_left": "name", "name_right": "district"})
...
def preprocess_raw_data(food_places_and_districts):
    ...
    food_places_and_districts: gpd.GeoDataFrame = food_places_and_districts[
        food_places_and_districts["cuisine"] != "coffee_shop"
    ]
    delivery_map = {
        "delivery": {
            "yes": True,
        ...
        "amenity": {"fast_food": "canteen", "food_court": "canteen"},
    }
    food_places_and_districts.replace(delivery_map, inplace=True)
    food_places_and_districts["delivery"] = food_places_and_districts.apply(
        (lambda x: random.choice([True, False])), axis=1  # noqa: S113
    )
    ...
    food_places_and_districts["open_time"] = (
        food_places_and_districts["opening_hours"].str.extract(open_close_time_pattern)[0].astype(int)
    )
    food_places_and_districts["close_time"] = (
        food_places_and_districts["opening_hours"].str.extract(open_close_time_pattern)[2].astype(int)
    )
    cuisine_map = {
        "kebab": "uzbekistan",
        "burger": "mexico",
